...
function renderResults(data){
  ...
  if( data.length > 0 ){
    $.each(data, function(i, payer) {
      ...
      var header = $('<h4 class="list-group-item-heading">'+ payer.name +'</h4>');
      var item_text = $('<ul class="list-group-item-text list-unstyled"></ul>');
      
      $.each(payer.receivable_accounts, function(i, account) {
        item_text.append('<li>Accout: ' + account.external_key + '</li>');
      });
