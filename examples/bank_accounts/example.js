...
function renderResults(data){
  ...
  if( data.length > 0 ){
    $.each(data, function(i, result) {
      ...
      var header = $('<h4 class="list-group-item-heading">Receivable account: '+ result.receivable_account_id +'</h4>');
      var item_text = $('<p class="list-group-item-text"></p>');
      
      item_text.append('Bank Account ID: ' + result.bank_account.id + '<br>');
      item_text.append('Account name: ' + result.bank_account.name + '<br>');
      item_text.append('Routing number: ' + result.bank_account.routing_number + '<br>');
      item_text.append('Account number: ' + result.bank_account.account_number);
