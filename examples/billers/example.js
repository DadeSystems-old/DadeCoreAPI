...
function renderResults(data){
  ...
  if( data.length > 0 ){
    $.each(data, function(i, biller) {
      var list_item = $('<li class="list-group-item"></li>');
      var header = $('<h4 class="list-group-item-heading">'+ biller.name +'</h4>');
      var item_text = $('<ul class="list-group-item-text list-unstyled"></ul>');
      
      item_text.append('<li>ID: ' + biller.id + '</li>');
      item_text.append('<li>External Key: ' + biller.external_key + '</li>');
